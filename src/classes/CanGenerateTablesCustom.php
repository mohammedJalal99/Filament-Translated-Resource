    ...
    protected function getResourceTableColumns(string $model): string
    {
        ...
        foreach ($schema->getColumns($table) as $column) {
            ...
            if ($type['name'] === 'boolean') {
                $columnData['type'] = TranslatedResource\Tables\Columns\IconColumn::class;
                $columnData['boolean'] = [];
            } else {
                $columnData['type'] = match (true) {
                    $columnName === 'image', str($columnName)->startsWith('image_'), str($columnName)->contains('_image_'), str($columnName)->endsWith('_image') => TranslatedResource\Tables\Columns\ImageColumn::class,
                    default => TranslatedResource\Tables\Columns\TextColumn::class,
                };

                if (in_array($type['name'], [
                        'string',
                        'char',
                    ]) && ($columnData['type'] === TranslatedResource\Tables\Columns\TextColumn::class)) {
